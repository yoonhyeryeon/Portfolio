...
let banner = {
    ...
    rollInit: function (newinterval) {
        ...
        let firstitem = document.querySelector('.notice-box li');
        if (firstitem) {
            firstitem.classList.add('currentroll');
        }

        let seconditem = document.querySelectorAll('.notice-box li')[1];
        if (seconditem) {
            seconditem.classList.add('nextroll');
        }

        document.querySelector('.notice-box li:last-child').classList.add('prevroll');
    ...
    rollNext: function () {
        ...
        if (document.querySelector('.currentroll').nextElementSibling) {
            ...
            document.querySelector('.notice-box li').classList.add('nextroll');
        }
    }
